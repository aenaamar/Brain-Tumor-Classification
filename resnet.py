...
BATCH_SIZE = 32
EPOCHS = 40 
LR = 5e-05 #the winning Learning Rate
DROPOUT_RATE = 0.2 #wining dropout rate
WEIGHT_DECAY = 1e-4
...
def run_experiment():
    ...
    best_model_path = os.path.join(WORKING_DIR, 'resnet50_best.pth')
    plot_path = os.path.join(WORKING_DIR, 'resnet_history.png')
    ...
    #zipping for kaggle
    print("\n--- Zipping Final Output Files ---")
    final_zip = os.path.join(WORKING_DIR, 'resnet_FINAL_outputs.zip')
    with zipfile.ZipFile(final_zip, 'w') as zipf:
        if os.path.exists(best_model_path):
            zipf.write(best_model_path, arcname='resnet50_best.pth')
        if os.path.exists(plot_path):
            zipf.write(plot_path, arcname='resnet_history.png')
            
    print(f"All done! Download your final zip file here: {final_zip}")
